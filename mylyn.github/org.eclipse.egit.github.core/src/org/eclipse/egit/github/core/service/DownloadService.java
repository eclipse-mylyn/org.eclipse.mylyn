 ...
 *
 * @see <a href="http://developer.github.com/v3/repos/downloads">GitHub downloads API documentation</a>
 */
public class DownloadService extends GitHubService {
	...
	public PageIterator<Download> pageDownloads(IRepositoryIdProvider repository, int start, int size) {
		PagedRequest<Download> request = createDownloadsRequest(repository, start, size);
	...
	/**
	 * Create download and set the content to be the content of given input stream. This is a convenience method that performs a
	 * {@link #createResource(IRepositoryIdProvider, Download)} followed by a {@link #uploadResource(DownloadResource, InputStream, long)}
	 * with the results.
	 *
	 * @param repository
	 * @param download
	 ...
	 * @throws IOException
	 */
	public DownloadResource createDownload(IRepositoryIdProvider repository, Download download, InputStream content,
	 ...
	 * @throws IOException
	 */
	public DownloadResource createDownload(IRepositoryIdProvider repository, Download download, File file)
			throws IOException {
		if (file == null) {
			throw new IllegalArgumentException("File cannot be null"); //$NON-NLS-1$
		}

		return createDownload(repository, download, Files.newInputStream(file.toPath()), file.length());
	}
}
