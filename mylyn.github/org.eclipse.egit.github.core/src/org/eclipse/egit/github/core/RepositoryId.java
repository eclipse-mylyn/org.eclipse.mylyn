...
public class RepositoryId implements IRepositoryIdProvider, Serializable {
	...
	/**
	 * Create repository from id. The id is split on the '/' character and the first two non-empty segments are interpreted to be the
	...
	public static RepositoryId createFromId(String id) {
		...
		return owner != null && owner.length() > 0 && name != null && name.length() > 0
				? new RepositoryId(owner, name)
						: null;
	}
	...
	public static RepositoryId createFromUrl(String url) {
		...
		try {
			return createFromUrl(new URI(url).toURL());
	...
	/**
	 * Create repository id with given owner and name. This constructor validates the parameters and throws an
	 * {@link IllegalArgumentException} if either is null or empty.
	 *
	 * @param owner
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof RepositoryId other)) {
			return false;
		}
