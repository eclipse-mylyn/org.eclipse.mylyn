...
public class IssueService extends GitHubService {
	 ...
	 */
	protected PagedRequest<Issue> createIssuesRequest(String repoId, Map<String, String> filterData, int start,
			int size) {
		StringBuilder uri = new StringBuilder(SEGMENT_REPOS);
		uri.append('/').append(repoId);
		uri.append(SEGMENT_ISSUES);
	 ...
	 * @return iterator
	 */
	public PageIterator<Issue> pageIssues(String user, String repository, Map<String, String> filterData, int start,
			int size) {
		verifyRepository(user, repository);
		String repoId = user + '/' + repository;
		PagedRequest<Issue> request = createIssuesRequest(repoId, filterData, start, size);
		return createPageIterator(request);
	}
	 ...
	 * @return iterator
	 */
	public PageIterator<Issue> pageIssues(IRepositoryIdProvider repository, Map<String, String> filterData, int start,
			int size) {
		String repoId = getId(repository);
		PagedRequest<Issue> request = createIssuesRequest(repoId, filterData, start, size);
		return createPageIterator(request);
	...
	public Comment createComment(String user, String repository, int issueNumber, String comment) throws IOException {
		return createComment(user, repository, Integer.toString(issueNumber), comment);
	}
	 ...
	 * @throws IOException
	 */
	public Comment createComment(String user, String repository, String issueNumber, String comment)
			throws IOException {
		verifyRepository(user, repository);
	 ...
	 * @throws IOException
	 */
	public List<SearchIssue> searchIssues(IRepositoryIdProvider repository, String state, String query)
			throws IOException {
		...
		StringBuilder uri = new StringBuilder(SEGMENT_LEGACY + SEGMENT_ISSUES + SEGMENT_SEARCH);
