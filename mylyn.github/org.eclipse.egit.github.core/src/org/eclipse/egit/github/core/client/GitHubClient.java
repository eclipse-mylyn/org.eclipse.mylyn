...
public class GitHubClient {
	 ...
	 * <p>
	 * This creates an HTTPS-based client with a host that contains the host value of the given URL prefixed with 'api' if the given URL is
	 * github.com or gist.github.com
	 *
	 * @param url
	 * @return client
	 */
	...
	/**
	 * Set the value to set as the user agent header on every request created. Specifying a null or empty agent parameter will reset this
	 * client to use the default user agent header value.
	 *
	 * @param agent
	 * @return this client
	 */
	...
	/**
	 * Set the value to set as the accept header on every request created. Specifying a null or empty header parameter will reset this
	 * client to use the default accept header value.
	 *
	 * @param header
	 * @return this client
	 */
	...
	protected HttpURLConnection createConnection(String uri) throws IOException {
		URL url;
		try {
			url = new URI(createUri(uri)).toURL();
	...
	public String getUser() {
		if (!userSet && user == null && credentials != null && credentials.startsWith(AUTH_TOKEN)) {
			userSet = true;
			try {
	...
	protected <V> V parseJson(InputStream stream, Type type, Type listType) throws IOException {
		...
		if (listType == null) {
			...
			try (BufferedReader reader = new BufferedReader(new InputStreamReader(
					stream, CHARSET_UTF8), bufferSize); JsonReader jsonReader = new JsonReader(reader)) {
	...
	protected boolean isError(final int code) {
		return switch (code) {
			case HTTP_BAD_REQUEST, HTTP_UNAUTHORIZED, HTTP_FORBIDDEN, HTTP_NOT_FOUND, HTTP_CONFLICT, HTTP_GONE, HTTP_UNPROCESSABLE_ENTITY, HTTP_INTERNAL_ERROR -> true;
			default -> false;
		};
	}
	...
	protected boolean isOk(final int code) {
		return switch (code) {
			case HTTP_OK, HTTP_CREATED, HTTP_ACCEPTED -> true;
			default -> false;
		};
	}
	...
	protected void sendParams(HttpURLConnection request, Object params) throws IOException {
		...
		if (params != null) {
			request.setRequestProperty(HEADER_CONTENT_TYPE, CONTENT_TYPE_JSON + "; charset=" + CHARSET_UTF8); //$NON-NLS-1$
	...
	/**
	 * Get response stream from GET to URI. It is the responsibility of the calling method to close the returned stream.
	 *
	 * @param request
	 * @return stream
	 * @throws IOException
	...
	/**
	 * Get number of requests that {@link #getRemainingRequests()} counts down from as each request is made
	 * <p>
