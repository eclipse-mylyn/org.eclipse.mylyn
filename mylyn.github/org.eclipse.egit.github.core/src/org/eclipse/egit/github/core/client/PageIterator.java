...
/**
 * Iterator for getting paged responses. Each call to {@link #next()} will make a client request for the next page of resources using the
 * URI returned from the previous request. The {@link #hasNext()} method can be used to determine if the last executed request contained the
 * location of the next page of results. This iterator also provides the next and last page numbers as well as the next and last URIs.
 *
 * @param <V>
	...
	public Collection<V> next() {
		...
		if (next != null) {
			if (nextPage < 1) {
				...
				try {
					request.setUri(new URI(next).toURL().getFile());
