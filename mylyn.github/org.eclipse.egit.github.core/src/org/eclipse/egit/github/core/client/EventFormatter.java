...
public class EventFormatter implements JsonDeserializer<Event> {
	...
	private final Gson gson = new GsonBuilder().registerTypeAdapter(Date.class, new DateFormatter())
	...
	public Event deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
		...
		try {
			EventPayload typedPayload = context.deserialize(rawPayload, payloadClass);
