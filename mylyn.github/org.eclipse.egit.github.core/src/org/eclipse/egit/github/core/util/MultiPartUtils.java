...
public final class MultiPartUtils {
	 ...
	 * @throws IOException
	 */
	public static HttpURLConnection post(String url, Map<String, Object> parts) throws IOException {
		try {
			HttpURLConnection post = (HttpURLConnection) new URI(url).toURL().openConnection();
		...
		try (BufferedOutputStream output = new BufferedOutputStream(
			...
			for (Entry<String, Object> part : parts.entrySet()) {
				...
				if (value instanceof InputStream input) {
					...
					output.write(part.getValue().toString().getBytes(CHARSET_UTF8));
				}
				output.write(newline);
			}
