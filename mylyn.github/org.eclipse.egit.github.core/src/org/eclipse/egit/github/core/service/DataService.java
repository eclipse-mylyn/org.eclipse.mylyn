 ...
 *
 * @see <a href="http://developer.github.com/v3/repos/git">GitHub data API documentation</a>
 */
public class DataService extends GitHubService {
	...
	public String createBlob(IRepositoryIdProvider repository, Blob blob) throws IOException {
		...
		uri.append(SEGMENT_GIT);
		uri.append(SEGMENT_BLOBS);
		ShaResource created = client.post(uri.toString(), blob, ShaResource.class);
	 ...
	 * @throws IOException
	 */
	public Tree createTree(IRepositoryIdProvider repository, Collection<TreeEntry> entries, String baseTree)
			throws IOException {
		final String id = getId(repository);
	 ...
	 * @throws IOException
	 */
	public Reference editReference(IRepositoryIdProvider repository, Reference reference, boolean force)
			throws IOException {
		final String id = getId(repository);
		if (reference == null) {
			throw new IllegalArgumentException("Reference cannot be null"); //$NON-NLS-1$
	...
	/**
	 * List tags for given repository. You can access to more information by using URL in Reference Object returned.
	 *
	 * @param repository
	 * @return non-null but possibly empty list of references
	 * @throws IOException
