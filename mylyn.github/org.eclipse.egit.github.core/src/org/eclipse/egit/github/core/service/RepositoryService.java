 ...
 *
 * @see <a href="http://developer.github.com/v3/repos">GitHub repository API documentation</a>
 * @see <a href="http://developer.github.com/v3/repos/forks">GitHub forks API documentation</a>
 */
public class RepositoryService extends GitHubService {
	 ...
	 * @return iterator over pages of repositories
	 */
	public PageIterator<Repository> pageRepositories(Map<String, String> filterData, int size) {
	 ...
	 * @return iterator over pages of repositories
	 */
	public PageIterator<Repository> pageRepositories(Map<String, String> filterData, int start, int size) {
	 ...
	 */
	public PageIterator<Repository> pageRepositories(String user, int start, int size) {
		if (user == null) {
	 ...
	 * @return iterator over pages of repositories
	 */
	public PageIterator<Repository> pageOrgRepositories(String organization, Map<String, String> filterData, int start,
			int size) {
	 ...
	 * @throws IOException
	 */
	public List<SearchRepository> searchRepositories(final String query, final String language, final int startPage)
			throws IOException {
		if (query == null) {
	 ...
	 * @throws IOException
	 */
	public List<SearchRepository> searchRepositories(final Map<String, String> queryParams, final int startPage)
			throws IOException {
		if (queryParams == null) {
	...
	public Repository createRepository(Repository repository) throws IOException {
		...
		return client.post(SEGMENT_USER + SEGMENT_REPOS, repository, Repository.class);
	}
	...
	public PageIterator<Repository> pageForks(IRepositoryIdProvider repository, int start, int size) {
		PagedRequest<Repository> request = createPagedForkRequest(repository, start, size);
		return createPageIterator(request);
	}
	 ...
	 * @throws IOException
	 */
	public Repository editRepository(String owner, String name, Map<String, Object> fields) throws IOException {
	 ...
	 * @throws IOException
	 */
	public Repository editRepository(IRepositoryIdProvider provider, Map<String, Object> fields) throws IOException {
	...
	/**
	 * Fork given repository into new repository. The new repository will be under the given organization if non-null, else it will be under
	 ...
	 * @throws IOException
	 */
	public List<Contributor> getContributors(IRepositoryIdProvider repository, boolean includeAnonymous)
			throws IOException {
		String id = getId(repository);
		StringBuilder uri = new StringBuilder(SEGMENT_REPOS);
		uri.append('/').append(id);
	...
	/**
	 * Merge branches in repository. This will merge branch and push result to the repository.
	 *
	 * @param repository
	 * @param merging
	 ...
	 * @since 5.3
	 */
	public RepositoryMergingResponse mergingBranches(IRepositoryIdProvider repository, RepositoryMerging merging)
			throws IOException {
		String id = getId(repository);
		StringBuilder uri = new StringBuilder(SEGMENT_REPOS);
		uri.append('/').append(id);
		uri.append(SEGMENT_MERGES);
		RepositoryMergingResponse result = client.post(uri.toString(), merging, RepositoryMergingResponse.class);
		if (result != null && result.getCommit() != null && result.getCommit().getSha() == null) {
