...
public class RepositorySearchWizardPage extends WizardPage implements IRepositorySearchResult {
	...
	public void createControl(Composite parent) {
		...
		repoListViewer.addSelectionChangedListener(event -> validate(repoListViewer));

		searchText.addModifyListener(e -> searchButton.setEnabled(searchText.getText().trim().length() != 0));
	...
	private void search(final String language, final String text, final TableViewer viewer) {
		...
		try {
					...
					PlatformUI.getWorkbench().getDisplay().syncExec(() -> {
						...
						setMessage(
								MessageFormat.format(
										Messages.RepositorySearchWizardPage_Found, Integer.valueOf(
												repositories.size())),
								INFORMATION);
			...
			setErrorMessage(MessageFormat.format(
					Messages.RepositorySearchWizardPage_Error, cause.getLocalizedMessage()));
