...
/**
 * The 'Builder' design pattern, for documents.  Implementations can build a specific kind of document
 * (such as HTML, or DocBook).
 * <p>
 * Note that many methods take {@link Attributes} to specify attributes of the element,
 * however most of these methods may take a more specific subclass of {@link Attributes}.
 * </p>
 * 
 * 
 * @author David Green
...
public abstract class DocumentBuilder {
	...
	/**
	 * Begin a document.  Calling this method is optional for some builders, however if called then it must be matched by a
	 * corresponding call to {@link #endDocument()}.
	 * 
	 * @see #endDocument()
	 */
	 ...
	 * Note that the provided attributes *may* be a subclass of the {@link Attributes} class, in which case the builder may attempt
	 * to apply the attributes specified.  Builders may choose to ignore attributes, and should fail silently if the given attributes
	 * are not as expected.
	 * 
	 * Each call to this method must be matched by a corresponding call to {@link #endBlock()}.
	 * 
	 * @param type
	 * @param attributes the attributes to apply to the block.  Callers may choose to specify a more specialized set of attributes by
	 * 						providing a subclass instance.
	 * 
	 * @see #endBlock()
	 */
	 ...
	 * 
	 * @param type
	 * @param attributes the attributes to apply to the span
	 * 
	 * @see #endSpan()
	 */
	 ...
	 * 
	 * @param level the level of the heading, usually 1-6
	 * @param attributes the attributes to apply to the heading
	 * 
	 * @see #endHeading()
	 */
	 ...
	 * 
	 * @param attributes the attributes, which may be an {@link ImageAttributes}.
	 * @param url the URL to the image, possibly relative
	 */
	 ...
	 * @param hrefOrHashName the url (which may be internal to the page if prefixed with a hash '#')
	 ...
	 * @param hrefOrHashName the url (which may be internal to the page if prefixed with a hash '#')
	 * @param text the text of the hyperlink
	 * @param title the title or alternative text which is typically displayed on mouse hover
	 * 
	 * @deprecated please use {@link #link(Attributes, String, String)} instead and place the title in the {@link Attributes#getTitle() attributes title}.
	 */
	@Deprecated
	...
	/**
	 * Create a hyperlink whose visual representation is an image.  Implementations must apply
	 * the attributes to the image tag.  For example, if the builder constructs HTML, the
	 * builder would emit <code>&lt;a href="...">&lt;img src="..."/>&lt;/a></code>.  In this case if the attributes define
	 * a css class then the resulting HTML should look like this: <code>&lt;a href="...">&lt;img src="..." class="..."/>&lt;/a></code>
	 * 
	 * @param linkAttributes the attributes of the link, which may be {@link LinkAttributes}
	 * @param imageAttributes the attributes of the image , which may be {@link ImageAttributes}
	 * @param href the url  (which may be internal to the page if prefixed with a hash '#')
	 * @param imageUrl the url of the image, which may be relative
	 */
	 ...
	 * a css class then the resulting HTML should look like this: <code>&lt;a href="...">&lt;img src="..." class="..."/>&lt;/a></code>
	 * 
	 * @param attributes the attributes of the image, which may be {@link ImageAttributes}
	 * @param href the url  (which may be internal to the page if prefixed with a hash '#')
	 * @param imageUrl the url of the image, which may be relative
	 * 
	 ...
	 * 
	 * @deprecated use {@link #link(Attributes, String, String)} instead and place the title in the {@link Attributes#getTitle() attributes title}.
	 */
	@Deprecated
	 ...
	 * Emit an acronym
	 * 
	 * @param text the acronym to emit
	 * @param definition the definition of the acronym, which is typically displayed on mouse hover
	 */
	...
	/**
	 * Create unescaped characters, usually with some embedded HTML markup.  Note that using this method
	 * causes the output to be HTML-centric
	 * 
	 * @param literal the literal characters to emit
	 */
	 ...
	 * 
	 * @param locator the locator that provides information about the current location in the markup
	 */
