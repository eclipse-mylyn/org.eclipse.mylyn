...
OLD_VERSION=`grep mylyn_feature.*version $ROOT/e3.4/site.xml | sed 's/.*\"\([^\"]*\)\"./\1/' | sed 's/-e33//' | head`
...
#mv $ROOT/e3.3 $BACKUP
mv $ROOT/e3.4 $BACKUP
test -e $ROOT/extras && mv $ROOT/extras $BACKUP || true
...
sed -i -e 's/http:\/\/download.eclipse.org\/tools\/mylyn\/update\/weekly\//http:\/\/download.eclipse.org\/tools\/mylyn\/update\//' $ROOT/e3.4/categories.xml
...
sed -i -e 's/http:\/\/download.eclipse.org\/tools\/mylyn\/update\/weekly\//http:\/\/download.eclipse.org\/tools\/mylyn\/update\//' $ROOT/incubator/categories.xml
