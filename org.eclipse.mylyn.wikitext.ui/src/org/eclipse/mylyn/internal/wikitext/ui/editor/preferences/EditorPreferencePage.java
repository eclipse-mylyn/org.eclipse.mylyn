...
public class EditorPreferencePage extends FieldEditorPreferencePage implements
	...
	public void createFieldEditors() {
		...
		for (Map.Entry<String, String> ent : prefs.getCssByBlockModifierType()
				.entrySet()) {
			String preferenceKey = Preferences.toPreferenceKey(ent.getKey(),
					true);
		...
		for (Map.Entry<String, String> ent : prefs.getCssByPhraseModifierType()
				.entrySet()) {
			String preferenceKey = Preferences.toPreferenceKey(ent.getKey(),
					false);
			addField(new StringFieldEditor(preferenceKey, ent.getKey(), group));
		}
		PlatformUI.getWorkbench().getHelpSystem().setHelp(getControl(), WikiTextUiPlugin.getDefault().getPluginId()+".preferences");
	}
