...
/**
 * A validator that can be used to validate regions of a document in an editor.
 * Delegates validation to a {@link MarkupValidator} and coordinates the translation of errors and warnings
...
public abstract class DocumentRegionValidator {
	...
	/**
	 * Validate a region of a document.  Validation results may be created as annotations on the annotation model,
	 * or as markers on the resource.
	 * 
	 * @param monitor the progress monitor
	 * @param document the document representing the content of the resource
	 * @param region the region of the document to validate
	 * 
	...
	protected void computeProblems(IProgressMonitor monitor, IDocument document, IRegion region) throws CoreException {
		...
		try {
			List<ValidationProblem> problems = delegate.validate(new SubProgressMonitor(monitor,totalWork/2), document.get(),region.getOffset(),region.getLength());
	 ...
	 * @param problems
	 */
	protected abstract void createProblems(IProgressMonitor monitor, IDocument document, IRegion region, List<ValidationProblem> problems) throws CoreException;
	 ...
	 * @throws CoreException
	 */
	protected abstract void clearProblems(IProgressMonitor monitor, IDocument document,IRegion region) throws CoreException;
	...
	public void setMarkupLanguage(MarkupLanguage markupLanguage) {
		...
		delegate = markupLanguage==null?null:WikiTextPlugin.getDefault().getMarkupValidator(markupLanguage.getName());
	}
