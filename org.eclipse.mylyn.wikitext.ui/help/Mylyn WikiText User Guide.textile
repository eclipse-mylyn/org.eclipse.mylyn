...
In the *Task Editor Extension* section select the markup language of choice.  Note that you may need to expand this section to see available choices.  To disable WikiText extensions to the Mylyn task editor select *None*. 

If your repository supports internal wiki links enter the link URL format under *Wiki Links*.  For example, you could enter @http://wiki.eclipse.org/{0}@.  Leaving this section blank will cause the default wiki URL for the repository to be used.  Note that the '@{0}@' portion of the URL will be replaced by the wiki-specific page identifier.
