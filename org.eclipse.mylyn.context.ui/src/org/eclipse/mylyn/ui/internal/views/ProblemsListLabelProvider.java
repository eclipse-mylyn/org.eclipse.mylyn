...
public class ProblemsListLabelProvider implements ITableLabelProvider, IColorProvider, IFontProvider {
    ...
    public Color getForeground(Object element) {
        if (element instanceof ProblemMarker) {
            String handle = MylarPlugin.getDefault().getStructureBridge(((ProblemMarker)element).getResource().getFileExtension()).getHandleForMarker(((ProblemMarker)element));
            return UiUtil.getForegroundForElement(MylarPlugin.getTaskscapeManager().getNode(handle));
        } else {
            return null;
        }  
    }
    ...
    public Color getBackground(Object element) {
        if (element instanceof ProblemMarker) {
            String handle = MylarPlugin.getDefault().getStructureBridge(((ProblemMarker)element).getResource().getFileExtension()).getHandleForMarker(((ProblemMarker)element));
            return UiUtil.getBackgroundForElement(MylarPlugin.getTaskscapeManager().getNode(handle));
        } else {
            return null;
        }
    }
