...
public abstract class AbstractFixtureTest {
	...
	private static List<GerritFixture>[] discoveredFixtures = new List[2];

	@Parameter(0)
	protected static GerritFixture fixture;
	...
	static Stream<Arguments> fixtureProvider(boolean defaultOnly) {
		if (discoveredFixtures[defaultOnly ? 1 : 0] == null) {
			...
			discoveredFixtures[defaultOnly ? 1 : 0] = (List<GerritFixture>) defFixture.discover(GerritFixture.class,
					"gerrit", defaultOnly);
			assertTrue(discoveredFixtures[defaultOnly ? 1 : 0].size() > 0, "No fixtures discovered");
			for (GerritFixture fixture : discoveredFixtures[defaultOnly ? 1 : 0]) {
