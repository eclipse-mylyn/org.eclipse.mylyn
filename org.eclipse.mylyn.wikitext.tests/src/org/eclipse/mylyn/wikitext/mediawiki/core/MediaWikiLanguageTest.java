...
public class MediaWikiLanguageTest extends TestCase {
	...
	public void testParagraph() {
		...
		assertTrue(Pattern.compile("<body><p>first para<br/>\\s*first para line2</p><p>second para</p><p>third para</p></body>").matcher(html).find());
	}
	...
	public void testHeadings() {
		for (int x = 1;x<=6;++x) {
			...
			System.out.println(html);
			assertTrue(Pattern.compile("<body><h"+x+" id=\"[^\"]+\">heading text</h"+x+"><p>first para<br/>\\s*first para line2</p><p>second para</p><p>third para</p></body>",Pattern.MULTILINE).matcher(html).find());
		}
	}
	...
	public void testPreformatted() {
		...
		System.out.println("HTML: \n"+html);
		assertTrue(Pattern.compile("<body><p>normal para</p><pre>preformatted\\s+more pre\\s+</pre><p>normal para</p></body>").matcher(html).find());
	}
	...
	public void testParagraphBreaksOnPreformatted() {
		...
		String html = parser.parseToHtml(markup);
		System.out.println(html);
		assertTrue(Pattern.compile("<body><p>a normal para</p><pre>preformatted\\s+p\\s+</pre><p>normal</p></body>",Pattern.MULTILINE).matcher(html).find());
	}
