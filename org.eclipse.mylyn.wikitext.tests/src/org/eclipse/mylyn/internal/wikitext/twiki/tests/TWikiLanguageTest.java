...
public class TWikiLanguageTest extends TestCase {
	...
	public void testHeadings() {
		for (String prefix: new String[] { "!!",""}) {
			for (int level = 1;level <= 6;++level) {
				...
				System.out.println(html);
				assertTrue(html.contains("<body><h"+level+" id=\"headingtext\">heading text</h"+level+"></body>"));
			}
		}
	}
	...
	public void testIconUrls() {
		for (String iconType: new String[] { "help", "tip", "warning" }) {
			...
			System.out.println(html);
			assertTrue(html.contains("<body><p>a <img border=\"0\" src=\"TWikiDocGraphics/"+iconType+".gif\"/> text</p></body>"));
		}
	}
}
