...
public class CompositeDegreeOfInterest implements IDegreeOfInterest {
    ...
    public void addEvent(InteractionEvent event) {
        for (IDegreeOfInterest info : infos) ((DegreeOfInterest)info).addEvent(event);
    }

    public List<InteractionEvent> getEvents() {
        Set<InteractionEvent> events = new HashSet<InteractionEvent>();
        for (IDegreeOfInterest info : infos) events.addAll(info.getEvents());
        return new ArrayList<InteractionEvent>(events); 
    }
    
    public float getValue() {
        float value = 0; 
        for (IDegreeOfInterest info : infos) value += info.getValue();
        return value;
    }

	public float getDecayValue() {
        float value = 0; 
        for (IDegreeOfInterest info : infos) value += info.getDecayValue();
        return value;
	}
    
    public float getEncodedValue() {
        float value = 0; 
        for (IDegreeOfInterest info : infos) value += info.getEncodedValue();
        return value;
    }
    ...
    public boolean isInteresting() {
        ...
        for (IDegreeOfInterest info : infos)  if (info.isInteresting()) isInteresting = true;
    ...
    public String toString() {
    	...
    	for (IDegreeOfInterest info : infos) result.append(info.toString());
    	result.append(")");
