...
public class ProductParser {
	...
	/**
	 * Parse the product page for the valid products that a bug can be logged
	 * for
	 * 
	 * @param repository
	 * @param in
	 *            The input stream for the products page
	...
	public List<String> getProducts(TaskRepository repository) throws IOException, ParseException, LoginException {
		...
		// ((HtmlTag)token.getValue()).isEndTag())
		// {
		// // compare the title to see if we think that there is a problem with
		// login
		// if((title.indexOf("login") != -1 || (title.indexOf("invalid") != -1
		// && title.indexOf("password") != -1) || title.indexOf("check e-mail")
		// != -1 || title.indexOf("error") != -1))
		...
		// }
		//		
		// // if we have no products and we suspect a login error, assume that
		// it was a login error
	 ...
	 * 
	 * @param products
	 *            The list of products to add this new product to
	 * 
